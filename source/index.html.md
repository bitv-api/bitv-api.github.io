...
- Pre-submitted: The order has been received but has not yet entered the matching queue.
- Submitting: The order is being submitted to the matching queue.
- Failed: The order was not successfully submitted.
- Place_timeout: The order placement request did not complete within the allowed time.
...
5. **API Key permissions** — whether this API Key has the permission required by the endpoint (read / trade). Check the permission settings on the API Key management page
...
5) Only some trading pairs are supported in the 5-level/20-level incremental market; 150-level snapshot increments support all trading pairs. <br>
...
| type            | string    | Order type, valid values: buy-market, sell-market, buy-limit, sell-limit, buy-ioc, sell-ioc, buy-limit-maker, sell-limit-maker |
...
| orderType | string | Order type, valid values: buy-market, sell-market, buy-limit, sell-limit, buy-ioc, sell-ioc, buy-limit-maker, sell-limit-maker |
