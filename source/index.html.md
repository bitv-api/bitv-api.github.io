...
您可以点击<a href='https://www.bitv.com/api/'>这里 </a> 创建 API Key。

每个用户可创建3组API Key，每个API Key可对应设置读取、交易两种权限。  
...
- 方法请求地址：即访问服务器地址https://api.bitv.com
...
* pre-submitted : 准备提交。订单已接收，尚未进入撮合队列。  
* submitting : 提交中。订单正在提交至撮合队列。  
* failed : 失败。订单未能成功提交。  
* place_timeout : 下单超时。下单请求未在规定时间内完成。  
...
A:  每个用户可创建3组API Key，每个API Key可对应设置读取、交易两种权限。   
...
| limit-order-max-buy-amt              | true     | float   | 现价买最大可买数量  |
| limit-order-max-sell-amt              | true     | float   | 现价卖最大可买数量     |
| buy-limit-must-less-than             | true     | float   |  限价单买入价格上限比例  |
...
| 字段名称 | 数据类型 | 描述                                                    |
| -------- | -------- | ------------------------------------------------------- |
| id       | long     | UTC 时间戳，单位秒，并以此作为此K线柱的id。⚠️ 数值本身不含时区偏移；但 K 线周期以 UTC+8 划分（见上方说明），故日 K 线的 id 落在 UTC+8 当日 0 时 |
...
<aside class="notice">返回的数据对象是一个对象数组，每个数组元素为一个 UTC 时间戳（单位毫秒）下的所有交易记录，这些交易记录以数组形式呈现。</aside>
...
<aside class="notice">返回的'data'对象是一个数组，每一个元素代表一个交易对的行情快照。</aside>
...
| count    | integer  | 交易次数（以滚动24小时计）               |
| bid      | float    | 当前最优买一价                           |
| bidSize  | float    | 当前最优买一量                           |
| ask      | float    | 当前最优卖一价                           |
| askSize  | float    | 当前最优卖一量                           |

<aside class="notice">本接口一次性返回全部交易对，数据量较大，不建议高频轮询。需要单个交易对的24小时行情，请使用「最近24小时行情数据」；需要单个交易对的聚合盘口，请使用「聚合行情（Ticker）」。</aside>
...
| created-at         | int      | 订单创建时间的 UTC 时间戳，单位毫秒                       |
| type               | string   | 订单类型                                                     |
...
| states     | true     | string | 查询的订单状态组合，使用','分割                              |                             | partial-canceled 部分成交撤销, filled 完全成交, canceled 已撤销 |
...
5）5档/20档逐笔增量行情仅支持部分交易对；150档快照增量支持全部交易对。<br>
...
| type            | string   | 订单类型，有效值：buy-market, sell-market, buy-limit, sell-limit, buy-ioc, sell-ioc, buy-limit-maker, sell-limit-maker |
...
| orderType       | string   | 订单类型，有效值： buy-market, sell-market,buy-limit,sell-limit,buy-ioc,sell-ioc,buy-limit-maker,sell-limit-maker |
