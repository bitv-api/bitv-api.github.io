...
toc_footers:
  - <a href='https://www.bitv.com/api/'>創建 API Key </a>
includes:
...
每個用戶可創建3組API Key，每個API Key可對應設置讀取、交易兩種權限。  
...
<aside class="notice">
每個 API Key 最多可綁定 10 個 IP 地址，僅支持單個主機地址，不支持網段。創建 API Key 時必須綁定 IP 地址。
</aside>
<aside class="warning">
...
* pre-submitted : 準備提交。訂單已接收，尚未進入撮合隊列。  
* submitting : 提交中。訂單正在提交至撮合隊列。  
* failed : 失敗。訂單未能成功提交。  
* place_timeout : 下單超時。下單請求未在規定時間內完成。  
...
**v1接口返回格式**：最上層欄位**按接口類型分為兩種**，並非所有接口都返回相同的欄位。

| 接口類型 | 最上層欄位 |
| -------- | ---------- |
| 行情類（`/market/*`） | `status`、`ch`、`ts`，以及 `data` 或 `tick` |
| 賬戶、訂單等其他接口 | `status`、`data` |
...
| ch       | string   | 接口數據對應的數據流。**僅行情類接口返回**                   |
| ts       | long     | 接口返回的UTC時間的時間戳，單位毫秒。**僅行情類接口返回**    |
| data     | object   | 接口返回數據主體                                             |
| tick     | object   | 行情數據主體。**部分行情接口（如 `GET /market/detail/merged`）用此欄位代替 `data`** |
...
| path      | string   | 發生錯誤的接口路徑。**僅網關層錯誤返回**     |
| timestamp | long     | 服務端時間戳，單位毫秒。**僅網關層錯誤返回** |

<aside class="notice">
錯誤響應有兩種結構，取決於請求在哪一層被拒絕：
</aside>

| 出錯層次 | 典型 err-code | 返回欄位 |
| -------- | ------------- | -------- |
| 網關層（鑑權、參數格式） | `token-not-valid`、`api-signature-not-valid`、`validation-format-error` | `status`、`data`、`err-code`、`err-msg`、`path`、`timestamp` |
| 業務層（賬戶、訂單等業務校驗） | `invalid-account-type` 等 | `status`、`data`、`err-code`、`err-msg` |

<aside class="notice">
<b>業務層錯誤不返回 <code>path</code> 與 <code>timestamp</code></b>，解析時請勿假定這兩個欄位一定存在。<code>err-msg</code> 的文案可能隨版本調整，程序判斷請以 <code>err-code</code> 為準。
</aside>
...
5. **API Key 權限** —— 該 API Key 是否已開通此接口所需的權限（讀取 / 交易 / 提幣），請到 API Key 管理頁檢查權限勾選
...
- 必須綁定 IP：在申請 API Key 時，必須綁定您的 IP 地址，以此來保證您的 API Key 僅能在您自己的 IP 上使用。
...
A:  每個用戶可創建3組API Key，每個API Key可對應設置讀取、交易兩種權限。   
...
| parameter-required                                           | 必填參數缺失，請檢查參數                                      |
...
| invalid-account-type                                         | 賬戶類型不支持該操作。下單須使用 spot 賬戶的 account-id      |
...
```json
{
  ...
  "ts": 1572362902044, //system update time
  "tick": {
    "bids": [
...
5）5檔/20檔逐筆增量行情僅支持部分交易對；150檔快照增量支持全部交易對。<br>
...
| type            | string   | 訂單類型，有效值：buy-market, sell-market, buy-limit, sell-limit, buy-ioc, sell-ioc, buy-limit-maker, sell-limit-maker |
...
| orderType       | string   | 訂單類型，有效值： buy-market, sell-market,buy-limit,sell-limit,buy-ioc,sell-ioc,buy-limit-maker,sell-limit-maker|
